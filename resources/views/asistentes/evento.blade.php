...
@section('titulo', "$evento->nombre - Asistentes")
@push('scripts')
...
<div class="container">
    ...
    <div class="text-right hidden-print" name="contenedorOpciones">
        ...
        <a role="button" class="btn btn-lg btn-success" href="{{ route('asistentesEventoPDF', $evento->id) }}" title="Descargar lista de asistentes aprobados."> <span class="far fa-file-pdf" aria-hidden="true"></span> Lista de asistentes aprobados</a>
...
//<script type="text/javascript">
    ...
    function cargarAsistentes(filtro = ""){     
        ...
        $.ajax({
            ...
            type: 'GET',
            url : "{{ url('events') }}/{{$evento->id}}"+"/assistants"+filtro,
            dataType: 'json',
            success: function(result){
                ...
                if(result.data.length > 0){
                    $.each(result.data, function (i, asistente) {
                        ...
                        evaluarElementosPanelAsistente(asistente, {{$evento->id}}, 'evento');
                    });
    ...
    function eliminarAsistente(asistente){
        $.ajax({
            headers: {'X-CSRF-TOKEN': $("[name='_token']").val()},
            type: 'DELETE',
            url : "{{ url('events') }}/{{$evento->id}}"+"/assistants/"+asistente,
            dataType: 'json',
            success: function(result){
                $("#myModal").modal("hide");
    ...
    function enviarMensaje(usuario){
        ...
        var ruta = "{{ url('events') }}/{{$evento->id}}/assistants/"+usuario+"/mail"; 
        $.ajax({
            headers: {'X-CSRF-TOKEN': $("[name='_token']").val()},
            type: 'POST',
            url : ruta,
